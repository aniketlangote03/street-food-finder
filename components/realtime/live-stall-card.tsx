...
interface LiveStallCardProps {
  stall: LiveStall
}

export function LiveStallCard({ stall }: LiveStallCardProps) {
  return (
      ...
      <Card className="h-full flex flex-col overflow-hidden transition-all duration-200 hover:shadow-lg hover:scale-[1.02]">
        ...
        <CardFooter className="flex flex-col items-start gap-2 text-sm">
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="font-semibold text-orange-500">Queue: {stall.current_queue_length || 0}</span>
          </div>
